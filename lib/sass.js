...
/**
 * Unofficial but implemented properties.
 */
...
function parse(tokens) {
  ...
  while (token = tokens.shift())
    switch (token[0]) {
      case 'continuation':
        require('sys').p('continuing ' + selector.string + ' with ' + token[1][1])
        selector = new Selector(selector.string + token[1][1])
        selectors.push(selector)
        // selector = new Selector(selector.string + token[1][1], selector.parent)
        // if (!selector.parent) 
        //   selectors.push(selector)
        break
      case 'selector':
      ...
      case 'property':
        ...
        if (unofficial.indexOf(token[1][1]) === -1)
          selector.properties.push(new Property(token[1][1], val))
        else
          selector.properties.push(new Property('-webkit-' + token[1][1], val),
                                   new Property('-moz-' + token[1][1], val))
        break
      case 'variable':
